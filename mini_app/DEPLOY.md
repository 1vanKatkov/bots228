...
## ⚡ Быстрый старт: загрузка без ручной настройки БД

**Базу данных создавать вручную не нужно.** При первом запуске приложение само:
- проверяет наличие базы из `DATABASE_URL`;
- при отсутствии базы создаёт её (на self-hosted PostgreSQL);
- создаёт таблицы (`users` и др.) через `CREATE TABLE IF NOT EXISTS`.

Что нужно сделать:
1. Выберите хостинг (Amvera, Railway, Render и т.д.).
2. Создайте проект и подключите репозиторий (папка `mini_app` или корень с `mini_app/`).
3. Добавьте сервис PostgreSQL в панели хостинга — получите `DATABASE_URL`.
4. Задайте переменные окружения: `DATABASE_URL`, `OPENROUTER_API_KEY`.
5. Задеплойте. Команда запуска: `uvicorn main:app --host 0.0.0.0 --port 8080` (или порт из `PORT`).

Миграции и ручной запуск скриптов не требуются.
...
Миграция не нужна: при старте приложение само создаёт таблицы. Если вы только добавили PostgreSQL в Amvera, база уже создана платформой — просто укажите выданный `DATABASE_URL` в переменных окружения.
...
Таблицы создадутся при первом запуске, миграция не нужна.
